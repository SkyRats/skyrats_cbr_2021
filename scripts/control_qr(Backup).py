...
class PrecisionLanding():
    def __init__(self, MAV):
        ...
        self.pid_x = PID(-0.004, 0, -0)
        self.pid_y = PID(0.004, 0, 0)
        ...
        self.pid_z.setpoint = 0.10 #Podemos mudar para um lidar (fazer um filtro)
        self.pid_w.setpoint = 0  # orientation

        # Limitacao da saida
        self.pid_x.output_limits = self.pid_y.output_limits = (-0.5, 0.5)
        ...
        while not rospy.is_shutdown():
            ...
            while(self.done == 0):
                ...
                if self.first_detection == 1:
                    if self.area_ratio == -1:
                        self.is_lost = 1
                
                if not self.is_lost and self.first_detection == 1:
                    if self.area_ratio < 0.0018:  # Drone ainda esta longe do H
                        if(flag == 0):
                            rospy.loginfo("Controle PID")
                            flag = 1
                        
                        self.velocity.x= self.pid_x(-self.center_y)
                        self.velocity.y = self.pid_y(self.center_x)
                        # PID z must have negative parameters
                        if(abs(self.velocity.x) < VEL_CERTO and abs(self.velocity.y) < VEL_CERTO):
                            self.velocity.z = self.pid_z(self.area_ratio)
                        else:
                            self.velocity.z = 0
